...
object ServiceBuilder {
    private const val URL = "http://watools.xyz/ankush_apis/event-tracking/v1/"
    ...
    fun <T> buildServiceWithURL(serviceType: Class<T>, baseURL: String): T {
        return Retrofit.Builder().baseUrl(baseURL).addConverterFactory(GsonConverterFactory.create()).client(okHttp.build()).build().create(serviceType)
    }
}